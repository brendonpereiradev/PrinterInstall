...
| Login por domínio | Autentica pelo Active Directory (UPN ou NetBIOS) via LDAP. Permite alterar o domínio padrão e servidor nas configurações (ícone ⚙️). |
...
1. **Login.** Entre com sua conta de domínio no formato `usuario@dominio` ou `DOMINIO\usuario`. Se precisar alterar o domínio padrão ou servidor LDAP, clique no ícone de configurações (⚙️) no canto superior. As credenciais não ficam salvas depois que você fecha o programa.
